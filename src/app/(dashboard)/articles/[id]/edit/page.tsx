...
export default function EditOrderPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      fetch(`/api/articles/${p.id}`)
        ...
        .then(data => {
          if (data && data.id) {
            ...
            if (data.sampleType) setSampleType(data.sampleType);
            if (data.pattern) setPattern(JSON.parse(data.pattern));
            if (data.sizeSystem) setSizeSystem(data.sizeSystem);
            if (data.sizePairs) setSizePairs(JSON.parse(data.sizePairs));
            
            if (data.type === 'bulk' && data.sizePairs) {
              setBulkSizePairs(JSON.parse(data.sizePairs));
            }

            if (data.upperMaterials) setUpperMaterials(JSON.parse(data.upperMaterials));
            if (data.liningMaterials) setLiningMaterials(JSON.parse(data.liningMaterials));
            if (data.logoLabels) setLogoLabels(JSON.parse(data.logoLabels));
            if (data.dringHooks) setDringHooks(JSON.parse(data.dringHooks));
            if (data.lacesTpu) setLacesTpu(JSON.parse(data.lacesTpu));
            if (data.threads) setThreads(JSON.parse(data.threads));
            if (data.insole) setInsole(JSON.parse(data.insole));
            if (data.toeCap) setToeCap(JSON.parse(data.toeCap));
            if (data.last) setLast(JSON.parse(data.last));
            if (data.mould) setMould(JSON.parse(data.mould));
            if (data.injectionDetails) setInjectionDetails(data.injectionDetails);
            if (data.packing) setPacking(JSON.parse(data.packing));
            if (data.photos) setPhotos(JSON.parse(data.photos));
            if (data.extraMaterials) setExtraMaterials(JSON.parse(data.extraMaterials));

            if (data.qty) setPoQty(data.qty);
            if (data.deliveryDate) setPoDelDate(data.deliveryDate);
            if (data.remarks) setPoRemarks(data.remarks);

            if (data.operations && data.operations.length > 0) {
              const activeIds = data.operations.map((o: any) => o.opId);
              const opMap = data.operations.reduce((acc: any, o: any) => { acc[o.opId] = o.assignedUserId; return acc; }, {});
              
              setSelectedSteps(prev => prev.map(p => {
                if (activeIds.includes(p.id)) {
                  return { ...p, selected: true, assignedUserId: opMap[p.id] };
                }
  ...
  const handleSubmitEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        party: party === 'NEW' ? newParty : party,
        articleName: articleName === 'NEW' ? newArticleName : articleName,
        type: orderType,
        ...
        sizeSystem,
        upperMaterials: JSON.stringify(upperMaterials),
        liningMaterials: JSON.stringify(liningMaterials),
        extraMaterials: JSON.stringify(extraMaterials),
        logoLabels: JSON.stringify(logoLabels),
        dringHooks: JSON.stringify(dringHooks),
        lacesTpu: JSON.stringify(lacesTpu),
        threads: JSON.stringify(threads),
        insole: JSON.stringify(insole),
        toeCap: JSON.stringify(toeCap),
        last: JSON.stringify(last),
        mould: JSON.stringify(mould),
        injectionDetails,
        packing: JSON.stringify(packing),
      ...
      if (orderType === 'bulk') {
        payload.sizePairs = JSON.stringify(bulkSizePairs);
      } else {
        payload.sizePairs = JSON.stringify(sizePairs);
      }

      const res = await fetch(`/api/articles/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Edit Order Details</h1>
        ...
        <form onSubmit={handleSubmitEdit} className="card-premium p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="space-y-6 border-t border-slate-200 pt-8">
            ...
            {orderType !== 'sample' && (
              <div className="border-t border-slate-200 pt-8 mt-8">
                <h3 className="font-bold text-slate-800 mb-4 uppercase tracking-wider text-sm flex items-center gap-2">Bulk Production Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">Total PO Quantity</label>
                    <input type="number" className="input-premium" value={poQty} onChange={e => setPoQty(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">Target Delivery Date</label>
                    <input type="date" className="input-premium" value={poDelDate} onChange={e => setPoDelDate(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">Remarks</label>
                    <input type="text" className="input-premium" value={poRemarks} onChange={e => setPoRemarks(e.target.value)} />
                  </div>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
